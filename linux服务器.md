...
- **套接字类型（Type）（int type）**
  ...
  - 面向连接的套接字（SOCK_STREAM）
    ...
    > 收发的套接字内部有缓冲，就是有字节数组。因此通过套接字传输的数据将保存在数组，因此数据可以填满缓冲一次被读取，也可以分段被读取，"不存在数据"边界问题（一次发送可能是单消息局部数据也可能是多消息整合数据，client无法区分）。当缓冲区占满后，套接字无法接受数据，停止传输。所以不存在数据丢失问题
...
- **TCP套接字中的I/O缓冲**
  ...
  1. ~I/O缓冲在每个TCP套接字中单独存在~ 应用程序层面缓冲，不由操作系统管理，在c中，如果使用FILE*管理socket，则标准I/O库会为每个对象分配I/O缓存（独立）。
  2. I/O缓冲在创建套接字时自动生成
...
- **软中断**	
  ...
  软中断根据目前看到的，应该包含以下几种：
  - 系统调用。主要是使用系统调用后在汇编层面使用int、sysenter等函数触发的中断。
  - 异常处理。类似于除零之类的异常触发的中断，我理解这部分也被称为trap中断（陷阱）。
  - 调试中断。debug断点。
  - 事件通知。通过singal信号量触发中断。
...
IO层面，就不一样。要记住，IO操作只有两个阶段(以接收数据为例)：
...
- 存在额问题
  ...
  1. 单个进程可监视的文件描述符的数量被限制，即监听的端口有限，这个数目的限制同时和FD_SETSIZE宏以及内存有很大关系，32/64位默认为1024个，～对于64位机默认为2048个～
...
1. 调用connect时内核发送一个SYN分节，～～ 若无响应则等待6s后再次发送一个，仍无响应则等待24s再发送一个，～～ 失败后按次幂重试，可调整，一般为$4^x$，最多重试3次，若总共等了75s后仍未收到响应则返回ETIMEDOUT错误；
...
零拷贝（Zero-Copy）是一种 `I/O` 操作优化技术，可以快速高效地将数据从文件系统移动到网络接口，而不需要在内核空间和用户空间之间进行复制。其在 `FTP` 或者 `HTTP` 等协议中可以显著地提升性能。
...
等服务器处理完后，应用程序主动调用close，发送FIN报文。
...
- 信号的检测
  - 进程陷入内核态后，有两种场景会对信号进行检测：
    - 进程从内核态返回到用户态前进行信号检测（比如，任意系统调用后）
    - 进程在内核态中，从睡眠状态被唤醒的时候进行信号检测（比如，等待IO完成中）
...
3. 当 timeout->tv_sec 和 timeout->tv_usec 之和大于 0 时，select 函数检测到绑定其上的 socket 有事件才会返回或者阻塞时长为 timeout->tv_sec + timeout->tv_usec 。
...
**定义：**惊群效应（thundering herd）是指多进程（多线程）在同时阻塞等待同一个事件的时候（休眠状态），如果等待的这个事件发生，那么他就会唤醒等待的所有进程（或者线程），但是最终却只能有一个进程（线程）获得这个事件的“控制权”，对该事件进行处理，而其他进程（线程）获取“控制权”失败，只能重新进入休眠状态，这种现象和性能浪费就叫做惊群效应。
