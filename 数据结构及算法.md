...
| 在结点T的左结点（L）的 **右子树（R）** 上做了插入元素 | 左右型 | 左右旋 | 相当于先对 L和R做一次左旋，就标成了左左型 |
| 在结点T的右结点（R）的 **右子树（R）** 上做了插入元素 | 右右型 | 左旋   ||
| 在结点T的右结点（R）的 **左子树（L）** 上做了插入元素 | 右左型 | 右左旋 | 相当于先对L和R做一次右旋，就变成了右右型 |
...
- 定义
  ...
  通常我们将一棵标准的二叉查找树称为2-节点，因为每个节点都只有一个键（关键字）和两个链接
...
**个人觉得主要流程上可以这么理解，首先从 root 节点遍历到应当插入的 leaf 节点，当插入后 leaf 节点会有 3 个关键码的时候向上分裂。3 个子树的话，左子树<val_1，val_1<中子树<val_2，val_2<右子树**
...
> **度数：**每个节点子节点的个数的最大值称为该树的度。
>
> **阶数：**一个节点的子节点数目最大值。 
>
> 这里其实是一个概念，通常情况下，阶应该等于度数+1，b树这里相等
